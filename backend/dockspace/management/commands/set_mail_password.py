"""
Module: management/commands/set_mail_password.py
Purpose: Django management command to set mail account passwords

Usage:
    python manage.py set_mail_password user@example.com --password "SecurePass123!"

Sets the password for a mail account using SHA512-CRYPT hashing (compatible with
Docker Mailserver/dovecot). Automatically regenerates DMS configuration files after
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            account = MailAccount.objects.get(email__iexact=email_arg)
        except MailAccount.DoesNotExist:
            raise CommandError(f"Mail account {email_arg} does not exist")

        email = (account.email or "").strip().lower()
        if not email:
            raise CommandError("Mail account must have an email to create postfix account entry")
