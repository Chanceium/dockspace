"""
Module: management/commands/test_oidc_flow.py
Purpose: Django management command to test end-to-end OIDC authorization flow

Usage:
    python manage.py test_oidc_flow [--username USER] [--email EMAIL] [--password PASS]

Creates a test user and OIDC client, then simulates a complete authorization code
flow using Django's test client. Useful for verifying OIDC configuration, group
access controls, and claim mapping.

Options:
    --username: Test account username (default: testuser)
    --email: Test account email (default: testuser@example.com)
    --password: Test account password (default: testpass123)
    --client-id: OIDC client ID (default: test-client)
    --client-secret: OIDC client secret (default: test-secret)
    --redirect-uri: OAuth redirect URI (default: http://localhost:8000/oidc/callback)
    --group-name: Required group for client access (default: oidc-test-access)

Plus additional options for profile fields (phone, address, etc.)
"""
import json
import urllib.parse
...
from dockspace.core.models import ClientAccess, MailAccount, MailGroup, MailQuota
import jwt
