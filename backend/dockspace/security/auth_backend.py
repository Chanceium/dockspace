"""
Module: security/auth_backend.py
Purpose: Custom Django authentication backends with TOTP support

Provides two authentication backends:
1. AccountUserBackend: Basic email/password authentication against MailAccount model
2. AccountUserWithTOTPBackend: Extends basic auth with TOTP two-factor authentication

Both backends create temporary Django User objects for session compatibility and OIDC
provider integration, while using MailAccount as the primary user model.

Key Features:
- Authenticates against MailAccount using SHA512-CRYPT password hashes
- Supports both username and email for login
- TOTP verification with configurable time window
- Creates linked Django User records for OIDC compatibility
"""
import crypt
...
class AccountUserBackend(BaseBackend):
    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        ...
        # Check account status - only allow active accounts to authenticate
